...
async def chat_reply_async(
    ...
    *,
    ...
    """Versão async nativa do chat (sem tools) via LiteLLM `acompletion`."""
    ...
    async def _call_llm(*, provider_settings: Settings, llm_model: str, llm_api_base: str | None) -> str:
        apply_litellm_env(provider_settings)
        base_kwargs: dict[str, Any] = {}
        if llm_api_base:
            base_kwargs["api_base"] = llm_api_base
    ...
    try:
        ...
        if _has_llm_config_values(fb_provider, fb_model, fb_key):
            ...
            try:
                text = await _call_llm(provider_settings=fb_settings, llm_model=str(fb_model), llm_api_base=fb_base)
            except Exception as exc2:  # noqa: BLE001
                logger.error("Falha no chat LLM (principal+fallback): %s", redact_secrets(str(exc2)))
                raise
        else:
